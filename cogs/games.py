...
class Games(commands.Cog):
    ...
    async def dice(self, context, number_of_sides: str):
        ...
        if not number_of_sides.isdigit():
            log_event(event_level=ERROR, event_details="FAILED: the <sides> parameter is not an integer")
            await message_instance.edit(content="the <sides> parameter must be an integer")
            failed = True

        if int(number_of_sides) > 900:
            log_event(event_level=ERROR, event_details="FAILED: the <sides> parameter is above the bounds of <900>")
            await message_instance.edit(content="the <sides> parameter must be less than or equal to 900")
            failed = True

        if int(number_of_sides) <= 0:
            log_event(event_level=ERROR, event_details="FAILED: the <sides> parameter is below the bounds of <0>")
        ...
        if failed is False:
            ...
            log_event(event_level=DEBUG, event_details="command successfully, " + output_message)
